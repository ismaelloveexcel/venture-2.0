...
_PLACEHOLDER_HINTS = ("...", "your", "example", "secret_", "sk-")
...
class RuntimeConfig:
    ...
    def from_env(cls) -> "RuntimeConfig":
        return cls(
            ...
            airtable_prospects_table=os.environ.get("AIRTABLE_PROSPECTS_TABLE", "Prospects").strip(),
            airtable_kpis_table=os.environ.get("AIRTABLE_KPIS_TABLE", "WeeklyKPIs").strip(),
            ...
            resend_from_name=os.environ.get("RESEND_FROM_NAME", "Venture OS").strip(),
            digest_to_email=os.environ.get("DIGEST_TO_EMAIL", "").strip(),
            auto_send_emails=os.environ.get("AUTO_SEND_EMAILS", "false").lower() == "true",
            followup_days=int(os.environ.get("FOLLOWUP_DAYS", "4")),
            revenue_target=int(os.environ.get("REVENUE_TARGET", "10000")),
            reply_intent_enabled=os.environ.get("REPLY_INTENT_ENABLED", "false").lower() == "true",
            reply_intent_min_prob=float(os.environ.get("REPLY_INTENT_MIN_PROB", "0.12")),
            reply_intent_volume_threshold=int(os.environ.get("REPLY_INTENT_VOLUME_THRESHOLD", "12")),
        )
...
def collect_config_warnings(cfg: RuntimeConfig) -> List[str]:
    ...
    if status["NOTION_API_KEY"]:
        if not cfg.notion_prospects_db:
            ...
            warnings.append("NOTION_PROSPECTS_DB invalid format (expected 32 hex chars)")
...
def collect_live_mode_blockers(cfg: RuntimeConfig) -> List[str]:
    ...
    blockers.extend(
        issue for issue in collect_config_warnings(cfg)
...
def preflight_messages(cfg: RuntimeConfig) -> List[Tuple[bool, str]]:
    ...
    status = build_config_status(cfg)
    for key, is_set in status.items():
        checks.append((True, f"{'[ok]' if is_set else '[warn]'} {key} {'set' if is_set else 'not set (feature disabled)'}"))
